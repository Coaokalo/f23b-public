...
lua tests/lua/independent_weapons_spec.lua
```
...
cmake -S experiments/independent-weapons -B build/weapons -A x64
cmake --build build/weapons --config Release
python -B tools/package_preview.py --runtime-input "PATH/TO/F23B-Burner-Assemblies-0eab91677b32.zip" --asset-overlay "PATH/TO/YF23-HORNET-VISUAL-OUTPUT" --weapons-dll "build/weapons/Release/F23B_Weapons.dll"
```
...
.venv/Scripts/python.exe tools/build_setup.py --aircraft-zip "PATH/TO/F23B-preview-2026-09-18.zip"
```
...
The independent weapon connection uses the installed generic weapon controller in
its primary cockpit context. It binds the custom IR descriptor and supplies the
Hornet-designated target direction; DCS owns missile guidance and simulation.
`config/releases/independent-weapons.json` pins the supported native binaries and
the reviewed helper DLL. A deliberate rebuild requires reviewing and updating the
helper hash before packaging. The flight bridge is a separate, unchanged DLL.
